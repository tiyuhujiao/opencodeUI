...
exports.fetchProviderUpstreamModels = fetchProviderUpstreamModels;
exports.normalizeUpstreamModels = normalizeUpstreamModels;
...
async function fetchProviderUpstreamModels(scope, draft, endpoint, options = {}) {
    const url = parseEndpoint(endpoint);
    const document = await (0, providerSettings_1.readProviderConfigDocument)(scope, options);
...
function findExistingProvider(config, providerId) {
    const providers = asObject(config.provider);
    return providers ? asObject(providers[providerId]) : undefined;
}
function resolveRequestHeaders(draft, existingOptions, options) {
...
async function resolveCredential(draft, existingOptions, options) {
...
function applyCredentialHeader(headers, credential, npm) {
...
async function readStoredApiKey(providerId, options) {
    const env = options.env ?? process.env;
...
function normalizeModel(value) {
    ...
    return {
        id,
        name,
        description: firstText(object, ["description", "summary"], 4096),
...
function emptyNormalizedModel(id, name) {
    return {
        id,
        name,
        description: "",
...
function firstText(object, keys, maxLength) {
...
function firstPositiveInteger(object, keys) {
    for (const key of keys) {
...
function formatUpstreamError(status, statusText, text) {
    let detail = "";
    try {
        const payload = asObject(JSON.parse(text));
        const error = asObject(payload?.error);
        detail = firstText(error ?? payload ?? {}, ["message", "detail", "error_description", "error"], 1000);
    }
    catch {
        detail = boundedText(text.replace(/\s+/g, " "), 1000);
    }
...
//# sourceMappingURL=providerUpstreamModels.js.map