...
exports.normalizeProviderCatalog = normalizeProviderCatalog;
exports.mergeConfiguredCatalogEntries = mergeConfiguredCatalogEntries;
exports.resolveStoredCredentialProviderIds = resolveStoredCredentialProviderIds;
exports.resolveStoredCredentialTypes = resolveStoredCredentialTypes;
const providerSettings_1 = require("./providerSettings");
function normalizeProviderCatalog(providerPayload, authPayload, configuredProviderIds = new Set()) {
...
function mergeConfiguredCatalogEntries(catalog, configured) {
...
function resolveStoredCredentialProviderIds(credentials, providers) {
    return new Set([...resolveStoredCredentialTypes(credentials, providers)]
        .filter(([, type]) => type === "api")
        .map(([providerId]) => providerId));
}
function resolveStoredCredentialTypes(credentials, providers) {
...
function groupProviderIdentities(providers, select) {
    const result = new Map();
    for (const provider of providers) {
...
function normalizeProviderList(value) {
    if (Array.isArray(value)) {
        return value.map(asRecord).filter((entry) => Boolean(entry));
    }
...
function catalogModelToDraft(id, value) {
    ...
    return (0, providerSettings_1.modelConfigToDraft)(id, configShape);
}
function normalizeCatalogCost(value) {
...
function normalizeCatalogModalities(value, capabilities) {
...
function readCapabilityModalities(value) {
    if (Array.isArray(value)) {
...
function readConnectedProviderIds(value) {
    if (!Array.isArray(value)) {
        return [];
    }
...
function normalizeAuthMethodList(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.flatMap((entry) => {
...
function normalizeAuthPrompts(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.flatMap((entry) => {
        ...
        const when = whenKey && (whenOp === "eq" || whenOp === "neq")
