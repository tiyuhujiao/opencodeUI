export type ReversedUnifiedPatch = {
  beforeText: string;
...
export function reverseUnifiedPatch(
  currentText: string,
  patchText: string,
  options: { currentExists: boolean }
): ReversedUnifiedPatch {
  ...
  const oldNoFinalNewline = parsed.hunks.some((hunk) =>
    hunk.lines.some((line) => line.noFinalNewline && line.kind !== 'add')
  );
  const newNoFinalNewline = parsed.hunks.some((hunk) =>
    hunk.lines.some((line) => line.noFinalNewline && line.kind !== 'remove')
  );
...
function parseUnifiedPatch(patchText: string): {
  hunks: PatchHunk[];
  beforeExists: boolean;
  afterExists: boolean;
} {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (observedOld !== hunk.oldCount || observedNew !== hunk.newCount) {
      throw new UnifiedPatchError(
        `Patch hunk counts do not match its content (${String(observedOld)}/${String(hunk.oldCount)}, ${String(observedNew)}/${String(hunk.newCount)}).`
      );
    }
    hunks.push(hunk);
  }
...
function isNullDeviceHeader(header: string): boolean {
...
function joinText(lines: string[], eol: '\n' | '\r\n', hasFinalNewline: boolean): string {
  if (lines.length === 0) {
