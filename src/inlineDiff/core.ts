...
export class InlineDiffConflictError extends Error {
  readonly code: 'STALE_DOCUMENT' | 'INVALID_HUNK';

  constructor(code: 'STALE_DOCUMENT' | 'INVALID_HUNK', message: string) {
    super(message);
...
type SnapshotInput = TextSnapshot | string;

type LineToken = {
  key: string;
  startOffset: number;
  endOffset: number;
  isFinalNewline: boolean;
};
...
type MutableChange = {
  beforeStartToken: number;
  beforeEndToken: number;
  afterStartToken: number;
  afterEndToken: number;
  additions: number;
  deletions: number;
};
...
export function createTextSnapshot(text: string): TextSnapshot {
  const firstEol = /\r\n|\n/u.exec(text)?.[0];
...
export function buildHunks(
  baselineInput: SnapshotInput,
  currentInput: SnapshotInput,
  options: BuildHunksOptions = {}
): ReviewHunk[] {
...
export function applyAcceptHunkToBaseline(
  baselineInput: SnapshotInput,
  currentInput: SnapshotInput,
  hunk: ReviewHunk
): TextSnapshot {
  ...
  const nextText =
    baseline.text.slice(0, hunk.beforeStartOffset) +
    hunk.afterText +
    baseline.text.slice(hunk.beforeEndOffset);
  return createTextSnapshot(nextText);
}

export function applyRejectHunkToCurrent(
  baselineInput: SnapshotInput,
  currentInput: SnapshotInput,
  hunk: ReviewHunk
): TextSnapshot {
  ...
  const nextText =
    current.text.slice(0, hunk.afterStartOffset) +
    hunk.beforeText +
...
function toSnapshot(input: SnapshotInput): TextSnapshot {
...
function buildMyersSteps(before: string[], after: string[], maxEditDistance: number): DiffStep[] | null {
...
function backtrackMyers(trace: Array<Map<number, number>>, before: string[], after: string[], distance: number): DiffStep[] {
  ...
  for (let currentDistance = distance; currentDistance > 0; currentDistance -= 1) {
    ...
    const previousDiagonal =
      diagonal === -currentDistance || (diagonal !== currentDistance && right < down)
        ? diagonal + 1
        : diagonal - 1;
...
function collectChanges(steps: DiffStep[], beforeTokens: LineToken[], afterTokens: LineToken[]): MutableChange[] {
  const changes: MutableChange[] = [];
  let beforeIndex = 0;
  let afterIndex = 0;
  let active: MutableChange | undefined;
...
function mergeChanges(changes: MutableChange[], mergeDistance: number): MutableChange[] {
  const merged: MutableChange[] = [];
...
function wholeDocumentChange(beforeTokens: LineToken[], afterTokens: LineToken[]): MutableChange {
  return {
    beforeStartToken: 0,
...
function makeHunk(
  ...
  beforeTokens: LineToken[],
  afterTokens: LineToken[]
): ReviewHunk {
  ...
  const kind: ReviewHunkKind = change.deletions === 0 ? 'insert' : change.additions === 0 ? 'delete' : 'replace';
  const identity = [
    baseline.hash,
    current.hash,
...
function tokenBoundary(tokens: LineToken[], tokenIndex: number, textLength: number): number {
  if (tokenIndex <= 0) {
    return 0;
  }
...
function contentTokensBefore(tokens: LineToken[], tokenIndex: number): number {
  let count = 0;
...
function countContentTokens(tokens: LineToken[]): number {
  return tokens.reduce((count, token) => count + (token.isFinalNewline ? 0 : 1), 0);
}

function assertHunkMatchesSnapshots(baseline: TextSnapshot, current: TextSnapshot, hunk: ReviewHunk): void {
